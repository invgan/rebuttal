...
def run_process(config):
    print('Running gpu {}, rec-iter {}, rec-rr {}, rec_lr {} ...'.format(config[0], config[1], config[2], config[3]))
    cmd = 'CUDA_VISIBLE_DEVICES={} python train.py --save_recs --cfg {} --debug --init_path {} --rec_iters {} --rec_rr {} --rec_lr {}'.format(config[0], config[4], config[5], config[1], config[2], config[3])
    os.system(cmd)
...
def main():
    ...
    with open(args.cfg) as f:
        ...
        rec_rr_list = sweep_params['REC_RR']
    ...
    for rec_iter in rec_iter_list:
        for rec_rr in rec_rr_list:
            for rec_lr in rec_lr_list:
                run_config = (counter%num_gpus, rec_iter, rec_rr, rec_lr, config_file, init_file)
